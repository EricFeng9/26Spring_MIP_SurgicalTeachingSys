...
GT_MARKER_MAP = {"GT1 / I级": "circle", "GT2 / II级": "diamond", "GT4 / III级": "square"}
GT_COLOR_MAP = {"GT1 / I级": "#f59e0b", "GT2 / II级": "#8b5cf6", "GT4 / III级": "#111827"}
FIT_COLOR_MAP = {True: "#16a34a", False: "#dc2626"}
...
def sample_box_dimension(rng_np, low, high, size, sampling_mode="truncnorm", sigma_divisor=6.0):
    low = float(low)
    high = float(high)
    if high <= low:
        return np.full(size, low, dtype=float)
    if sampling_mode == "uniform":
        return rng_np.uniform(low, high, size=size)

    mu = 0.5 * (low + high)
    sigma = max((high - low) / max(float(sigma_divisor), 1e-6), 1e-9)
    arr = rng_np.normal(mu, sigma, size=size)
    mask = (arr < low) | (arr > high)
    tries = 0
    while np.any(mask) and tries < 24:
        arr[mask] = rng_np.normal(mu, sigma, size=int(mask.sum()))
        mask = (arr < low) | (arr > high)
        tries += 1
    if np.any(mask):
...
def sample_gt_boxes(
    gt_boxes, *, beta, tau_s, gamma, p0, d0, t0_ms, color_factor,
    n_each, seed, sampling_mode="truncnorm", sigma_divisor=6.0
):
    rng_np = np.random.default_rng(seed)
    rows = []
    for box in gt_boxes:
        power = sample_box_dimension(rng_np, box.power_min, box.power_max, n_each, sampling_mode, sigma_divisor)
        time_ms = sample_box_dimension(rng_np, box.time_min, box.time_max, n_each, sampling_mode, sigma_divisor)
        spot_um = sample_box_dimension(rng_np, box.spot_min, box.spot_max, n_each, sampling_mode, sigma_divisor)
        score = score_formula(
            power,
            time_ms,
            spot_um,
        ...
        rows.append(pd.DataFrame({
            ...
            "score": score,
            "note": box.note,
            "gt_sampling_mode": "截断正态" if sampling_mode == "truncnorm" else "均匀采样",
        }))
    return pd.concat(rows, ignore_index=True)


def prepare_gt_samples(gt_boxes, n_each, seed, sampling_mode="truncnorm", sigma_divisor=6.0):
    rng_np = np.random.default_rng(seed)
    prepared = []
    for box in gt_boxes:
        prepared.append({
...
def prepare_space_samples(rng_box: Range3D, *, n_samples, seed):
    rng_np = np.random.default_rng(seed)
    return {
        "power": rng_np.uniform(rng_box.power_min, rng_box.power_max, size=n_samples),
        "time_ms": rng_np.uniform(rng_box.time_min, rng_box.time_max, size=n_samples),
        "spot_um": rng_np.uniform(rng_box.spot_min, rng_box.spot_max, size=n_samples),
    }


def evaluate_region_uniformity(space_samples, *, beta, tau_s, gamma, p0, d0, t0_ms, color_factor, t1, t2, t3):
    score = score_formula(
        space_samples["power"], space_samples["time_ms"], space_samples["spot_um"],
        beta=beta, tau_s=tau_s, gamma=gamma, p0=p0, d0=d0, t0_ms=t0_ms, color_factor=color_factor
    )
    pred = classify_scores(np.asarray(score), t1, t2, t3)
    counts = pd.Series(pred).value_counts().reindex(REGION_ORDER).fillna(0).astype(float).values
    freqs = counts / max(len(pred), 1)
    mad = float(np.mean(np.abs(freqs - 0.25)))
    uniformity = max(0.0, 1.0 - mad / 0.375)
    return uniformity, freqs
...
def random_search_formula_params(prepared_boxes, *, color_factor, t1, t2, t3,
                                 ...
                                 prepared_space=None, uniformity_weight=0.25):
    rng_np = np.random.default_rng(seed)
    best = None
...
def make_isosurfaces(
    rng: Range3D, *, beta, tau_s, gamma, p0, d0, t0_ms, color_factor, t1, t2, t3, grid_n,
    gt_df=None, show_gt_points=False, gt_overlay_mode="按GT盒子", gt_points_cap=400, gt_point_size=5
):
    ...
    if show_gt_points and gt_df is not None and len(gt_df):
        overlay_df = gt_df.copy()
        if len(overlay_df) > gt_points_cap:
            overlay_df = overlay_df.sample(gt_points_cap, random_state=42)

        if gt_overlay_mode == "按命中情况":
            for fit_ok, sub in overlay_df.groupby("fit_ok"):
                fig.add_trace(go.Scatter3d(
                    x=sub["power_mW"], y=sub["time_ms"], z=sub["spot_um"],
                    mode="markers",
                    marker=dict(
                        size=gt_point_size,
                        color=FIT_COLOR_MAP[bool(fit_ok)],
                        symbol="diamond" if fit_ok else "x",
                        opacity=0.92,
                        line=dict(color="white", width=1),
                    ),
                    name=("GT点：命中" if fit_ok else "GT点：未命中"),
                    legendgroup=("gt_fit_ok" if fit_ok else "gt_fit_bad"),
                    hovertemplate=(
                        "<b>%{customdata[0]}</b><br>目标区域=%{customdata[1]}<br>预测区域=%{customdata[2]}<br>"
                        "score=%{customdata[3]:.3f}<br>功率=%{x:.1f} mW<br>时间=%{y:.1f} ms<br>光斑=%{z:.1f} μm<extra></extra>"
                    ),
                    customdata=np.stack([
                        sub["gt_name"], sub["target_region"], sub["pred_region"], sub["score"]
                    ], axis=1),
                ))
        else:
            for gt_name, sub in overlay_df.groupby("gt_name"):
                fig.add_trace(go.Scatter3d(
                    x=sub["power_mW"], y=sub["time_ms"], z=sub["spot_um"],
                    mode="markers",
                    marker=dict(
                        size=gt_point_size,
                        color=GT_COLOR_MAP.get(gt_name, "#111827"),
                        symbol=GT_MARKER_MAP.get(gt_name, "circle"),
                        opacity=0.92,
                        line=dict(color="white", width=1),
                    ),
                    name=f"GT点：{gt_name}",
...
with st.sidebar.expander("GT采样设置", expanded=True):
    gt_sampling_label = st.radio("GT采样方式", ["截断正态", "均匀采样"], index=0, horizontal=True)
    gt_sampling_mode = "truncnorm" if gt_sampling_label == "截断正态" else "uniform"
    sigma_divisor = st.slider(
        "截断正态：σ = 区间宽度 / N",
        2.0, 10.0, 6.0, 0.5,
        disabled=(gt_sampling_mode != "truncnorm"),
    )
    if gt_sampling_mode == "truncnorm":
        st.caption("GT 盒子边界较粗时，推荐用截断正态：中心更可信，边界权重更低。")
    else:
        st.caption("均匀采样把 GT 盒子内所有参数视为同等可信。")
...
with st.sidebar.expander("自动拟合公式参数（固定当前阈值）", expanded=False):
    ...
    opt_space_n = st.slider("均匀性评估采样数", 1000, 30000, 6000, 500)
    uniformity_weight = st.slider("均匀性权重", 0.0, 1.0, 0.25, 0.05)
    st.caption("目标函数 = (1-权重)×GT盒子平均命中率 + 权重×四区均匀性。权重越大，越优先让四个区域接近 25%/25%/25%/25%。")
    ...
    auto_fit_params = st.button("自动修改公式参数，同时兼顾GT命中与区域均匀")
...
with st.sidebar.expander("可视化精度", expanded=False):
    ...
    show_gt_on_surface = st.checkbox("在等值面图上叠加GT点", value=True)
    gt_overlay_mode = st.radio("GT点着色方式", ["按GT盒子", "按命中情况"], index=0, horizontal=True)
    gt_points_cap = st.slider("等值面图最多显示GT点数", 50, 2000, 500, 50)
    gt_point_size = st.slider("GT点大小", 3, 12, 6, 1)
...
if auto_fit_params:
    prepared = prepare_gt_samples(GT_BOXES, n_each=opt_gt_each, seed=st.session_state.seed + 12345, sampling_mode=gt_sampling_mode, sigma_divisor=sigma_divisor)
    prepared_space = prepare_space_samples(ranges, n_samples=opt_space_n, seed=st.session_state.seed + 24680)
...
region_freqs_current = (region_counts / sample_n).reindex(REGION_ORDER).astype(float).values
current_mad = float(np.mean(np.abs(region_freqs_current - 0.25)))
current_uniformity = max(0.0, 1.0 - current_mad / 0.375)

gt_df = sample_gt_boxes(GT_BOXES, beta=beta, tau_s=tau_s, gamma=gamma, p0=p0, d0=d0, t0_ms=t0_ms, color_factor=color_factor, n_each=gt_n_each, seed=st.session_state.seed + 7, sampling_mode=gt_sampling_mode, sigma_divisor=sigma_divisor)
...
for col, title, value in [
    ...
    (c7, "区域均匀性", f"{current_uniformity:.1%}"),
]:
    with col:
...
gt_sampling_desc = gt_sampling_label + (f"（σ=区间宽度/{sigma_divisor:.1f}）" if gt_sampling_mode == "truncnorm" else "")
st.markdown(
    f"<div class='small-note'>当前参数：颜色={color_label}，k_color={color_factor:.3f}，β={beta:.3f}，τ={tau_s:.3f} s，γ={gamma:.3f}，P₀={p0:.1f} mW，d₀={d0:.1f} μm，t₀={t0_ms:.1f} ms，P95-P5={spread:.4f}，GT采样={gt_sampling_desc}，GT盒子平均命中率={mean_box_fit:.1%}，区域均匀性={current_uniformity:.1%}</div>",
    unsafe_allow_html=True,
)
if last_fit_result:
    st.info(
        "最近一次自动拟合结果："
        f"GT盒子平均命中率={last_fit_result['box_mean']:.1%}，总体命中率={last_fit_result['overall']:.1%}，区域均匀性={last_fit_result.get('uniformity', float('nan')):.1%}，综合目标={last_fit_result.get('combined', float('nan')):.1%}；"
        ...
        + (f"；区域占比={', '.join([f'{lab}:{v:.1%}' for lab, v in zip(REGION_ORDER, last_fit_result.get('region_freqs', []))])}" if last_fit_result.get('region_freqs') else "")
    )
...
with tab2:
    st.plotly_chart(
        make_isosurfaces(
            ranges,
            beta=beta, tau_s=tau_s, gamma=gamma, p0=p0, d0=d0, t0_ms=t0_ms, color_factor=color_factor,
            t1=t1, t2=t2, t3=t3, grid_n=iso_grid_n,
            gt_df=gt_df, show_gt_points=show_gt_on_surface, gt_overlay_mode=gt_overlay_mode,
            gt_points_cap=gt_points_cap, gt_point_size=gt_point_size,
        ),
        use_container_width=True,
    )
    st.caption("这里显示的是 3 个阈值对应的等值面；勾选后可把 GT 采样点叠加在等值面之间，直观看到它们落在各分区边界的哪里。")
with tab3:
...
with tab4:
    ...
    st.caption("命中率 = 在对应 GT 盒子里按当前采样方式（均匀 / 截断正态）采样后，落入目标区域的比例。自动拟合按钮会固定当前阈值，搜索公式参数，同时兼顾这些命中率和四区均匀性。")
with tab5:
...
st.markdown(
    ...
    **使用建议**
    ...
    - GT 盒子边界如果不准，推荐把左侧 **GT采样方式** 切到 **截断正态**，让中心参数权重更高。
    - 需要提高 GT 命中率但又不想四区失衡时，优先点左侧 **自动修改公式参数**。它固定当前阈值，只搜索公式参数，并使用当前 GT 采样方式，同时把四区均匀性一起纳入目标。
    - GT拟合图用于看“当前物理计算层是否接住已知参考点”；视觉形态是否像真实光斑，仍由你的渲染层负责。
    - 若想直接看 GT 点与三张等值面的空间关系，可在左侧打开 **在等值面图上叠加GT点**。
    """
)
