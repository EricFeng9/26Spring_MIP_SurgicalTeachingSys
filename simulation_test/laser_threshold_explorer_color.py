...
st.caption("把红/黄/绿三种光颜色作为乘法系数并入热作用公式，在功率/时间/光斑三维参数空间里调整 3 个阈值，观察分区边界是否“好看”。")
...
st.caption("点击左侧“自动求解：四区等体积”后，会用均匀随机采样估计 score 的 25% / 50% / 75% 分位数，将三维参数盒子近似均分成四块。切换颜色后，公式值范围和阈值范围会一起变化。")
...
st.markdown(
    ...
    **使用建议**
    ...
    - 切换红/黄/绿后，再点击一次“自动求解：四区等体积”，比较不同颜色下阈值的位置变化。
    - 如果你想让黄光更“敏感”，就提高 `k_yellow`；如果想模拟红光穿透优势但基础热效应更低，可先保留较低 `k_red`，后续再单独加入介质透过修正。
    """
)
